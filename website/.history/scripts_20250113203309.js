document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.toggle-password').forEach((toggle) => {
    toggle.addEventListener('click', function () {
      const input = this.previousElementSibling; // Access the input directly before the toggle button
      if (input && input.type === 'password') {
        input.type = 'text';
        this.textContent = '🙈'; // Hide password icon
      } else if (input) {
        input.type = 'password';
        this.textContent = '👁'; // Show password icon
      }
    });
