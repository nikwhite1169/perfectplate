document.addEventListener('DOMContentLoaded', () => {
  // Attach toggle functionality to all elements with the 'toggle-password' class
  document.querySelectorAll('.toggle-password').forEach(toggle => {
    toggle.addEventListener('click', function () {
      const input = this.previousElementSibling; // Get the associated input
      if (input.type === 'password') {
        input.type = 'text';
        this.textContent = '🙈'; // Change the icon for 'hide'
      } else {
        input.type = 'password';
        this.textContent = '👁'; // Change the icon for 'show'
      }
    });
  });
});
